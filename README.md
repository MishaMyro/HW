...
The catalog table uses a stable `ID` column. The ID should match the folder name, for example `unit-2-test`.

## Adding a new page

1. Create a new folder, for example `unit-2-test/`.
2. Add `unit-2-test/index.html` and optional `unit-2-test/style.css`.
3. Add a new row to the table in the root `index.html` with ID, title, date created, and link.
